...
## 2025-10-30 13:40:20
- タグライン二行目をグリッドレイアウト化し、テキストを中央揃えに保ったままドットアニメーションを右端に寄せるよう調整。`hero-tagline__text` を追加し、`hero-tagline__ellipsis` の配置を右寄せに変更（`index.html` / `style.css`）。

## 2025-10-30 13:42:01
- 前項の変更をリクエストにより取り下げ、タグラインの二行目を従来の `inline-flex` レイアウトへ戻してドットとテキストを一連の並びに復元。あわせてモバイル用 `--hero-tagline-gap` を元の値に戻し、余計な差分を解消（`index.html` / `style.css`）。
