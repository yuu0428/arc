(() => {
  ...
  const evaluateLogoContrast = () => {
    ...
    if (!sampledColor) {
      logo.classList.add('is-sampling');
      const elementBeneath = document.elementFromPoint(sampleX, sampleY);
      logo.classList.remove('is-sampling');
      sampledColor = resolveBackgroundColor(elementBeneath) || resolveBackgroundColor(document.body);
    }
  ...
  window.addEventListener('resize', () => window.requestAnimationFrame(evaluateLogoContrast));
  ...
  function initializeCircularNav() {
    ...
    function updateWheel(force = false) {
      ...
      items.forEach((item, index) => {
        ...
        const itemDistance = Math.max(navState.radius * 0.68, navState.radius - 80);
        const displayAngle = angle + navState.displayOffset;
        const transform = `translate(-50%, -45%) rotate(${displayAngle.toFixed(3)}deg) translateX(${itemDistance.toFixed(3)}px) rotate(180deg) scale(${scale.toFixed(3)})`;
